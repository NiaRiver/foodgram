...
class User(AbstractUser):
    email = models.EmailField(
        unique=True, max_length=settings.MAX_LENGTH_EMAIL, verbose_name="Email"
    )
    ...
    first_name = models.CharField(
        max_length=settings.MAX_LENGTH_NAME, verbose_name="First Name"
    )
...
class ShortenedRecipeURL(models.Model):
    recipe = models.OneToOneField(
        Recipe, on_delete=models.CASCADE, related_name="shortened_url"
    )
    short_code = models.CharField(
        max_length=6, unique=True, default=generate_short_code
    )
